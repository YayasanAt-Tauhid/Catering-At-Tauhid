...
export default function KitchenPrintTicket() {
  ...
  const handlePrint = () => {
    const itemsToPrint = filteredItems.filter(item => selectedItems.has(item.menu_item_id));
    
    if (itemsToPrint.length === 0) {
      return;
    }

    const groupedPrint = itemsToPrint.reduce((acc, item) => {
      if (!acc[item.category]) {
        acc[item.category] = [];
      }
      acc[item.category].push(item);
...
${Object.entries(groupedPrint).map(([category, items]) => `
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Cetak daftar menu yang harus disiapkan</p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-muted-foreground">Tidak ada pesanan untuk tanggal ini</p>
          </CardContent>
        </Card>
      ) : (
        <>
                      ...
                      {items.map((item) => (
                        ...
                        >
                          <Checkbox 
                            ...
                            onCheckedChange={() => handleSelectItem(item.menu_item_id)}
                          />
