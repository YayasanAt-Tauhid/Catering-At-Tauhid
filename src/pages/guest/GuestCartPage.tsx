...
import { UtensilsCrossed, Plus, Minus, Trash2, ShoppingCart, ArrowLeft, ArrowRight } from 'lucide-react';

export default function GuestCartPage() {
  const { cart, updateCartQuantity, removeFromCart, clearCart, getCartTotal, getCartItemCount } = useApp();
    ...
    <div className="min-h-screen gradient-warm">
      ...
      <header className="container mx-auto px-4 py-6">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/guest/menu')}>
          ...
          <Card className="hidden lg:block">
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between">
                <span className="text-muted-foreground">Subtotal ({getCartItemCount()} item)</span>
                <span className="font-bold">Rp {getCartTotal().toLocaleString('id-ID')}</span>
              </div>
              <Button 
                variant="hero" 
                size="lg" 
